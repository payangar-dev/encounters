...
13. **Portal invasion's global lock + cooldown is a single timestamp, not a per-portal map.** Because only one invasion may run at a time globally, `NetherPortalInvasionEvent.lastInvasionEndTick` plus the lock variable are sufficient to gate the next one. After any invasion ends, every portal in the world is on the same `netherPortalInvasionPortalCooldownTicks` cooldown — intentional simplification, acceptable for solo/small co-op.
...
20. **Breaking the portal during an invasion stops it.** Once per second, `PortalInvasion.tick()` checks that `anchorKey` is still a `Blocks.NETHER_PORTAL`; if not (frame block destroyed, or any chain neighbour update extinguished the portal), `onPortalBroken()` runs the same teardown as `onAbandoned()` — releases the wave lockdown, discards the magma-bomb caster, fires the network end packet and frees the global lock. Already-spawned mobs stay alive on purpose: the player still has to deal with what came through. Subsequent waves are skipped.
