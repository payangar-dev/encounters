...
Minecraft **1.21.1** mod (Fabric + NeoForge, Java 21). Configurable in-world encounter events: a thunderstorm bolt may awaken an "overcharged" group of mobs, nether portals may disgorge raids, etc. Tuning (toggles, cooldowns, group sizes) lives in Cloth Config JSON5 partitioned under `<config>/encounters/`; spawn rosters and per-mob NBT live in vanilla datapacks under `data/encounters/spawn_pools/`.

## Stack

- MultiLoader-Template (jaredlll08), Mojang + Parchment mappings
- Fabric Loom **1.8.13**
- Cloth Config **15.0.140** (multi-loader, AutoConfig + Jankson serializer)
...
- **common/**: all gameplay logic, config, event API, cinematic system, Cloth Config annotations, datapack-loaded spawn pools (`event/pool/`, default JSONs under `resources/data/encounters/spawn_pools/`)
...
Common pulls Cloth Config's `-neoforge` artifact at `compileOnly`: Mojang-mapped since NeoForge 1.20.5+, matches our NeoForm setup. The platform-agnostic `cloth-config` jar is compiled against Fabric intermediary mappings and cannot be used in `common`. Loader modules pull the loader-specific runtime artifact.

The mod runs on **both sides**: server owns event logic and mob spawning; client only needs the jar for parity (and the Cloth Config GUI).
...
| Cloth Config + JSON5 partitions (capability blocks, list widgets, legacy backup) | ✅ done | `config/EncountersConfig.java` |
...
- **Portal trigger paths** (natural): `PortalScanner` (subclass of `EncounterScanner`) runs on every server tick, sweeps a 25×25×Y cube around each living overworld player every `portal.scanner.scanIntervalTicks` ticks, dedupes blocks to portal sites, rolls per site against `portal.scanner.triggerChance`, and calls `NetherPortalInvasionEvent.forceTrigger(level, site, face)`. Debug: `/encounters trigger nether_portal_invasion` requires a portal within 16 blocks of the source position.
- **Scanner abstraction**: `event/EncounterScanner.java` is the base class for periodic scanner-driven triggers (`PortalScanner`, `SkirmishScanner`). It owns the common check chain (overworld, enabled, tick interval, trigger chance > 0) and the DEBUG-level rejection logging. Sub-classes override `canTrigger(level, config)` to pick which capability gates they read (concurrency cap, post-event cooldown, day-only…) and `scan(level, config)` for their event-specific candidate logic + commit strategy.
...
- **Config GUI**: `config/ConfigScreenBuilder.create(parent)` is called from each loader's ModMenu / NeoForge config screen factory; the screen is fully derived from `@ConfigEntry.*` annotations on `EncountersConfig` and its per-event sub-configs, generated by `AutoConfig.getConfigScreen`.
...
Split between two distinct surfaces: **tuning** (Cloth Config) and **rosters** (datapacks).

### Tuning: Cloth Config (JSON5 via Jankson)

- **Files**: partitioned under `<config-dir>/encounters/`, one JSON5 per category (`general.json5`, `lightning.json5`, `portal.json5`, `skirmish.json5`), written by `PartitioningSerializer.wrap(JanksonConfigSerializer::new)`.
- **Backend**: Cloth Config v15 AutoConfig: annotations drive both serialization and GUI.
- **GUI**: `AutoConfig.getConfigScreen(EncountersConfig.class, parent)` opens a four-tab screen (one per category). Each capability block renders as a `@ConfigEntry.Gui.CollapsibleObject` sub-section inside its event tab.
- **Composition by capability** (one block = one concern): four reusable POJO blocks compose per-event Settings:
    - `ScannerSettings` (triggerChance, scanIntervalTicks): for scanner-driven events
    - `ConcurrencySettings` (maxConcurrent, minDistanceBetween): for events that cap how many run at once and how close they may stand
    - `CooldownSettings` (cooldownTicks): for events that impose a world-wide cooldown after one ends
    - `CohesionSettings` (enabled, radius): for events that pull stragglers back to the group leader
- **Code drives composition, not config.** A scanner that wants a concurrency cap consults `ActiveEncounterTracker.activeCount(...)`; one that wants a cooldown consults `ActiveEncounterTracker.cooldownElapsed(...)`. The presence of `ConcurrencySettings` / `CooldownSettings` in `*Settings` only exposes the tunings of what the code already reads. Adding a block to a config without wiring its reads in code does nothing; wiring code reads without exposing the block in config gives no GUI for the value.
- **Per-event Settings** (`LightningSettings`, `PortalSettings`, `SkirmishSettings`, `GeneralSettings`) include only the capability blocks the event's code consumes. Defaults specific to one event (e.g. portal's `cooldownTicks = 12000` vs skirmish's `6000`) are set via static helper methods because Cloth instantiates sub-configs through the no-arg constructor.
- **Capability matrix** (what each event's code reads: config follows):

  | Event | Scanner | Concurrency | Cooldown | Cohesion |
  |---|---|---|---|---|
  | Lightning Overcharge | ❌ (passive event-hook driven) | ❌ | ❌ | ✓ |
  | Nether Portal Invasion | ✓ | ✓ | ✓ | ✓ |
  | Patrol Skirmish | ✓ | ✓ | ✓ | ❌ (custom faction leash) |

- **Scope**: primitives (booleans, ints, doubles) + `List<String>` for the biome whitelist (rendered as a native editable list widget by Cloth: add/remove/edit in GUI).
- **Reload contract**: `EncountersConfig.holder().registerSaveListener(...)` invokes `EncounterRegistry.invalidateAll()` whenever the GUI saves; datapack reloads invalidate the same caches independently via `EncounterPoolsManager`.
- **Legacy rename**: `LegacyConfigMigrator.run()` runs once before `AutoConfig.register`. If `<config-dir>/encounters.json5` exists and carries a pre-migration flat field marker (`lightningOverchargeChance`, `netherPortalInvasionMaxConcurrent`, `patrolSkirmishMobsPerSide`), it is renamed to `encounters.json5.old` and Cloth regenerates a fresh schema. Custom values are not auto-migrated; the user consults the `.old` file to port them by hand.
- **`general.mobsDropEquipment`** (global, default `false`): when false, `EncounterSpawner.spawn` forces `Mob#setDropChance(slot, 0)` after `finalizeMobSpawn`, overriding vanilla defaults. When `true`, drop chances revert to vanilla defaults (~8.5% main hand). Equipment drop chances are no longer carried in the spawn pool NBTs.
...
- **Mod-presence filter**: `NbtModFilter` walks every loaded NBT looking for `modid:path` strings whose namespace is neither `minecraft` nor a loaded mod (via `Services.PLATFORM.isModLoaded`). Filtered entries are silently skipped at `MobRoster.resolve`, so a single default datapack covers any modpack permutation (EpicFight, Iron's Spells, SimplySwords, EternalNether, Alex's Mobs entries auto-skip when their mod is absent).
- **Migration from pre-datapack configs**: legacy `xxxMobs` fields were captured in the original YACL config; that whole file is now backed up as `encounters.json5.old` on the first launch after the Cloth migration (see "Legacy rename" above). Custom user entries from the legacy format are not migrated; the user has to port them to a datapack.
...
- Lang keys: Cloth AutoConfig uses `text.autoconfig.encounters.category.<name>` for tabs and `text.autoconfig.encounters.option.<categoryName>.<fieldPath>` for options (with `.@Tooltip` suffix for hover text). Sub-objects use dot notation in the path (e.g. `portal.tracker.maxConcurrent`)
...
1. **Loom 1.8.13 ↔ Cloth Config 15.0.140: confirmed compatible.** No Loom bump required. The previous YACL pin (3.6.6, because 3.7.0+ required Loom 1.10+) is gone: Cloth Config doesn't depend on a specific Loom version. The `common` module pulls the `-neoforge` artifact at `compileOnly` because the platform-agnostic `cloth-config` jar is compiled against Fabric intermediary mappings and clashes with our NeoForm setup. Loader modules pull the loader-specific runtime artifact (`-fabric` / `-neoforge`).
2. **Spawn rosters live as datapack JSON, NOT in the Cloth Config.** Datapacks give us hot-reload (`/reload`), JSON-native NBT (no SNBT-string escape gymnastics), free third-party overrides, and a much better UX for power users. Editing rosters means editing JSON, not the in-game GUI: by design. The historical YACL JSON5 writer bug (`quilt-parsers 0.2.x` `JsonWriter.string()` static-array mutation that corrupted saves) was an additional motivator at the time, but is no longer relevant since we moved off YACL.
...
13. **`ActiveEncounterTracker` owns two separate concerns, exposed via separate APIs.** (1) **Present cinematics**: `activeCount(level, eventId)` / `nearestActiveDistance(level, eventId, pos)` over a single `CopyOnWriteArraySet`, scanned linearly (at N ≤ ~20 active cinematics, strictly faster than bucketed indices). (2) **Post-event cooldown**: `cooldownElapsed(level, eventId, cooldownTicks)` reads from a `Map<(dim, eventId), lastEndTick>` populated automatically by `unregister(cinematic)`. The two concerns are never conflated in the API: a scanner that wires only a concurrency cap calls only `activeCount`; one with only a cooldown calls only `cooldownElapsed`. Portal invasion settings: `portal.concurrency.maxConcurrent` (default 1), `portal.concurrency.minDistanceBetween` (default 256, enforced only in `PortalScanner` so the debug command may force adjacent invasions), `portal.cooldown.cooldownTicks` (default 12000). The events no longer carry `canScannerTrigger` / `releaseLock` / `lastXEndTick`: the scanner reads `ActiveEncounterTracker` directly and the cooldown timestamp is bumped automatically when a cinematic's `finishAndRelease` calls `unregister(this)`.
...
Architecture is ready for additional events: drop a new class implementing `Cinematic`, expose a `public static final ResourceLocation POOL_ID`, ship a `data/encounters/spawn_pools/<id>.json` default, and reuse `EncounterSpawner` + `EncounterAllies` + `MobRoster` + `ActiveEncounterTracker`. For scanner-driven events, subclass `EncounterScanner` and override `canTrigger` (compose `ActiveEncounterTracker.activeCount` / `cooldownElapsed` as needed) + `scan` (candidate logic + commit). For event-hook driven events (lightning), wire the listener manually. The scanner check chain (enabled / interval / triggerChance) and DEBUG rejection logging are inherited from `EncounterScanner` for free.
