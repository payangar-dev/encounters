...
| Global equipment-drop toggle (`mobsDropEquipment`, default false) | ✅ done | `EncountersConfig` + `LightningOverchargeEvent.spawnOne` |
...
- **Annotation-driven GUI**: `@AutoGen` + `@TickBox`/`@IntSlider`/`@DoubleSlider`/`@ListGroup`, split across two categories (`general`, `lightning_overcharge`)
...
10. **Equipment drop override runs after `finalizeMobSpawn`.** The `setDropChance` loop in `spawnOne` must come after both `loadEntityRecursive` (which applies NBT drop chances) and `finalizeMobSpawn` (which may set vanilla defaults for mobs without custom NBT). Running it earlier would let either path clobber our override.
